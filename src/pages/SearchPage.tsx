...
import { useRecommended } from "@/hooks/useLocalSections";
...
const SearchPage = () => {
  ...
  const { play, setQueue, currentSong, isPlaying, togglePlay, playlists, playlistSongs, addSongToPlaylist, createPlaylist, toggleLike, isLiked } = usePlayerStore();
            ...
            {showSuggestions && query.length >= 1 && autocompleteSuggestions.length > 0 && (
              ...
              >
                {autocompleteSuggestions.map((item, i) => (
                  ...
                  >
                    {item.coverUrl ? (
                      <SafeImage src={item.coverUrl} alt="" referrerPolicy="no-referrer" className={`w-10 h-10 object-cover flex-shrink-0 ${item.type === "artist" ? "rounded-full" : "rounded-lg"}`} />
                    ) : (
                      ...
                      >
                        <div className="w-full h-full rounded-full overflow-hidden bg-background">
                          {(artistPhotos[artist.name] || artist.cover) ? (
                            <SafeImage src={artistPhotos[artist.name] || artist.cover} alt={artist.name} referrerPolicy="no-referrer" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                          ) : (
                      ...
                      {artistCards.map((artist) => (
                        ...
                        >
                          ...
                          >
                            <div className="w-full h-full rounded-full overflow-hidden bg-background">
                              {artist.coverUrl ? (
                                <SafeImage src={artist.coverUrl} alt={artist.name} referrerPolicy="no-referrer" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                              ) : (
                                <div className="w-full h-full flex items-center justify-center" style={{ background: "hsl(var(--foreground) / 0.03)" }}>
            ...
            >
              <div className="flex items-center gap-3 p-4 border-b" style={{ borderColor: "hsl(var(--border) / 0.15)" }}>
                {addToPlaylistRelease.coverUrl && (
                  <SafeImage src={addToPlaylistRelease.coverUrl} alt="" referrerPolicy="no-referrer" className="w-12 h-12 rounded-xl object-cover" style={{ boxShadow: "0 2px 12px hsl(0 0% 0% / 0.15)" }} />
                )}
                <div className="flex-1 min-w-0">
